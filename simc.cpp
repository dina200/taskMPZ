...
int itransmax;
array<1,evemax,bool> waitevent;
array<1,evemax,int> maxevent;
listl userlist;
ptransact trans;
...
plists stlist;
plisth histlist;
array<min_signal,max_signal,plistt> signlist;
array<1,evemax,plistt> ass, waitl;
...
array<min,max,t>::array(t* other){
    for (int i=0;i<max+1-min;i++)mem[i]=other[i];
   }

/*template <int min, int max, typename t>
t& array<min,max,t>::oator[](int index) }*/
...
template <int size>
char* cstring<size>::asarray(void)	{
    return str;
    }

template<int min, int max>
ostream& operator<<(ostream& os,array<min,max,char> a)
{
...
outfile<<"<TH width=15%><EM>2016</EM></TH>"<<endl;
outfile<<"</TR>"<<endl;
outfile<<"</THEAD>"<<endl;
outfile<<"<TR>"<<endl;
...
typedef array<1,5,char> array5;
typedef array<1,linelength,char> arrayl;


static void fillpacked(arrayl& a,
                         array5 fild5)
  {
    int i,
    j;
...
/*    arrayl workline,
    ...
    fillpacked(board,as(array5,"+----"));
    fillpacked(midle,as(array5,".    "));
    midle[1]='I';
    midle[linelength]='I';
    fillpacked(limiter,as(array5,"+...."));
    {
       histogram& with = *hist;
...
/*    output << " Using dialog (Y - yes / any key - not) ?";
    ...
    if ((ch=='Y')||(ch=='y'))
     ...
     } else
     {
       ...
       input >> file_name/* >> NL */;
       char* ext=".html";
       strcat(file_name.asarray(),ext);
       outfile.open(file_name.asarray(),ios::out);
     }
